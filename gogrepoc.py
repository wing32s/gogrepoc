...
if sys.version_info < MIN_PYTHON:
    print("Your Python version is not supported, please update to 3.10+")
    sys.exit(1)
...
For detailed help on a specific command:
  ...
  Example: %(prog)s download -h
    ...
    if args.command in {'update', 'download', 'backup', 'import', 'verify'}:
...
def main(args):
    ...
    match args.command:
        ...
        case 'update':
            ...
            cmd_update(args.os, args.lang, args.skipknown, args.updateonly, not args.full, args.ids, args.skipids, args.skiphidden, args.installers, args.resumemode, args.strictverify, args.strictdupe, args.md5xmls, args.nochangelogs)
        case 'download':
            if args.id:
                args.ids = [args.id]
            if not args.os:
            ...
            if args.downloadlimit is not None:
                args.downloadlimit = args.downloadlimit * 1024.0 * 1024.0
